...
void free_min(node *tree, size_t to_free, size_t *amt) {
  if (tree->file_size >= to_free && tree->type == T_DIR) {
    if (tree->file_size < *amt || *amt == 0UL)
      *amt = tree->file_size;
  }

  for (int i = 0; i < tree->c_count; i++) 
    free_min(&(tree->children[i]), to_free, amt);
}
...
int main(int argc, char **argv) {
  ...
  size_t m = 0;
  printf("NEED: %lu\n", 30000000UL - (70000000LU - root->file_size));
  free_min(root, 30000000UL - (70000000LU - root->file_size), &m);
  printf("FREE THIS: %lu\n", m);
